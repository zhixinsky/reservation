...
请求
链接
http://ip:port /inter/sendTemplateNormalSMS
...
mobile	String	7.将所有参数拼装成json串后
； 8.将json串进行AES加密；
9.加密后的byte数组通过post直接传输;
手机号 (必填)最多500个
...
3、如果请求信息中有压缩标示，返回的数据也是压缩过的，需要解压缩；[非必须步骤] data = GZIP.dcom(data);

...
function clampBookAheadDays(value) {
    ...
    if (!Number.isFinite(n)) return DEFAULT_STORE_RULES.bookAheadDays;
    return Math.min(3, Math.max(1, Math.round(n)));
}

function addDaysToYmd(ymd, offsetDays) {
...
/** 可预约日期区间：今天起共 bookAheadDays 天（1=仅今天，3=今天/明天/后天） */
function isDateWithinBookAhead(targetDate, today, bookAheadDays) {
    const days = clampBookAheadDays(bookAheadDays);
    const maxYmd = addDaysToYmd(today, days - 1);
    return targetDate >= today && targetDate <= maxYmd;
}
...
function normalizeStoreRules(store) {
    ...
    return {
        ...
        bookAheadDays: clampBookAheadDays(base.bookAheadDays),
